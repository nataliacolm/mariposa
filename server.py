...
class Server:
    ...
    def calcShuttleSlots(timeStart, timeEnd, avgUse, dist):
        hourStart = int(timeStart.split(":", 1)[0]) - 1
        hourEnd = int(timeEnd.split(":", 1)[0]) - 1
        trip_type = "one-way"
        i = 1
        for hour in range(hourStart, hourEnd):
            shuttleEmissions = CarbonCar().calculate_co2(
                ...
                pax_in_car=avgUse[hour],
                trip_type=trip_type,
            )
            print("  Shuttle Option", i,"\n    Est. Arrival Time: ", hour + 1, "\n    Carbon Emission Points: ", shuttleEmissions, "\n    Est. # of Passengers: ", avgUse[hour], "\n")
            i = i+1
        return i - 1
